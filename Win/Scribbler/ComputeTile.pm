...
my $Default = 'lightzero';
my @Icons = qw/lightzero lightrom lightcurr lightposdif lightnegdif lightabsdif/;
my %Calls = (
	lightzero => 'RefLightZero', lightrom => 'RefLightRom', lightcurr => 'RefLightLight',
	lightposdif => 'ComputePosDif', lightnegdif => 'ComputeNegDif', lightabsdif => 'ComputeAbsDif'
);
my $Index = 0;
...
sub _createEditWindow {
	...
	my $mw = $scribbler->mainwindow->Toplevel(-title => $scribbler->translate('Perform a computation.'), -background => 'BLACK');
	$mw->withdraw;
